...
class G5K(Provider):
    def init(self, config, force_deploy=False):
        ...
        site_roles = self._build_site_roles()
    ...
    def before_preintsall(self, env):
        ...
        if env['eths'][EXTERNAL_IFACE] == 'veth0':
            self._exec_command_on_nodes(
                    ...
                    'Creating a virtual interface for veth0')
    ...
    def _compute_resources(self):
        ...
        for cluster, templates in self.config['resources'].items():
            ...
            resources.setdefault(site, {})
            resources[site][cluster] = roles_goal
    ...
    def _make_reservation(self):
        ...
        for site, rs in self.resources.items():
            for cluster, roles in rs.items():
    ...
    def _check_nodes(self,
        ...
        wanted_nodes = 0
        for site, rs in resources.items():
            for cluster, roles in rs.items():
    ...
    def _deploy(self):
        ...
        if len(undeployed) > 0:
            logging.error("%d nodes where not deployed correctly:" % len(undeployed))
            for n in undeployed:
                logging.error(n)
    ...
    def _build_site_roles(self):
        ...
        for site, resource in self.resources.items():
            ...
            for cluster, rs in resource.items():
            ...
            logging.info("Site: %s - Roles: %s" % (site, pf(roles)))
    ...
    def _get_cluster_nics(self, cluster):
        ...
        nics = EX5.get_resource_attributes(
            '/sites/%s/clusters/%s/nodes' % (site, cluster))['items'][0]['network_adapters']
